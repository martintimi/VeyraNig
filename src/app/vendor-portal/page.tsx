        ...
        <div className="min-h-screen flex flex-col bg-[var(--bg-primary)]">
          ...
          <div className="flex-1 flex flex-col md:flex-row">
            ...
            <main className="flex-1 p-6 sm:p-10 lg:p-12 overflow-y-auto space-y-8">
              ...
              {/* ======================================================== */}
              {/* VIEW 7: WHATSAPP SOCIAL COMMERCE & DM CLOSER TOOLKIT */}
              {/* ======================================================== */}
              {activeView === 'whatsapp' && (
                <div className="space-y-8 animate-fadeIn max-w-7xl">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-mono-luxury uppercase tracking-widest text-emerald-500 font-bold">
                          Social Commerce Assistant
                        </span>
                      </div>
                      <h1 className="font-editorial text-3xl sm:text-4xl font-bold text-[var(--text-primary)] mt-1">
                        WhatsApp DM Closer & Status Kit
                      </h1>
                      <p className="text-xs text-[var(--text-secondary)] font-mono-luxury mt-1 max-w-2xl">
                        Post real photos on your WhatsApp Status and close DMs in 10 seconds with automatic 3D sizing and Paystack Escrow checkout.
                      </p>
                    </div>
                  ...
                  <div className="p-5 rounded-3xl surface-card border border-[var(--border-subtle)] space-y-3">
                    ...
                    <div className="flex items-center gap-3 overflow-x-auto pb-1 scrollbar-none">
                      {allProducts.slice(0, 8).map((p) => {
                        const isSelected = selectedWhatsAppProductId === p.id;
                        return (
                          <div
                            key={p.id}
                            onClick={() => setSelectedWhatsAppProductId(p.id)}
                  ...
                  {(() => {
                    const activeProduct = allProducts.find(p => p.id === selectedWhatsAppProductId) || allProducts[0];
                    ...
                    const generatedMessage = `Hi ${whatsappCustomerName || 'there'}! ✨ Thanks for reaching out about the ${activeProduct.name} (₦${activeProduct.price.toLocaleString()}).\n\nTo see how this fits your exact body measurements in 3D and order with 24h Lagos delivery, tap this direct link: ${productLink}\n\n${whatsappCustomNote ? `Note: ${whatsappCustomNote}` : ''}`;
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                        ...
                        <div className="lg:col-span-5 p-6 sm:p-8 rounded-3xl surface-card border border-[var(--border-subtle)] space-y-4 shadow-xl">
                          ...
                          <div className="space-y-2 pt-2">
                            <button
                              onClick={() => {
                                alert('Photo saved! Post this exact image to your WhatsApp Status.');
                              }}
                              className="w-full py-3.5 rounded-full bg-[var(--text-primary)] text-[var(--bg-primary)] font-mono-luxury uppercase text-xs font-bold hover:opacity-90 transition-all shadow-md flex items-center justify-center gap-2"
                            >
                              <Download className="h-4 w-4" />
                              <span>Save Status Photo</span>
                            </button>

                            <p className="text-[10px] font-mono-luxury text-[var(--text-muted)] text-center">
                              Post this photo on WhatsApp Status $\rightarrow$ buyers see price & 3D fit tag immediately!
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="lg:col-span-7 p-6 sm:p-8 rounded-3xl surface-card border border-[var(--border-subtle)] space-y-5 shadow-xl">
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-mono-luxury uppercase text-[var(--gold-accent)] font-bold">
                              1-Tap WhatsApp DM Closer
                            </span>
                            <span className="text-[10px] font-mono-luxury text-zinc-400">Copy & Paste in 1-Click</span>
                          </div>
                          ...
                          <div className="p-4 rounded-2xl bg-[var(--bg-primary)] border border-[var(--border-subtle)] text-xs font-mono-luxury leading-relaxed space-y-2">
                            <div className="text-[10px] text-[var(--gold-accent)] font-bold uppercase">Ready-to-Send WhatsApp Text:</div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] font-mono-luxury">
                              <button
                                onClick={() => {
                                  ...
                                  alert('Sizing script copied!');
                                }}
                                className="p-2.5 rounded-xl bg-[var(--bg-primary)] border border-[var(--border-subtle)] hover:border-[var(--gold-accent)] text-left truncate text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
                              >
                              ...
                              <button
                                onClick={() => {
                                  const text = `You can pay securely via Paystack debit card or direct bank transfer into our escrow account: ${productLink}`;
                                  navigator.clipboard.writeText(text);
                                  alert('Payment script copied!');
                                }}
                                className="p-2.5 rounded-xl bg-[var(--bg-primary)] border border-[var(--border-subtle)] hover:border-[var(--gold-accent)] text-left truncate text-[var(--text-secondary)] hover:text-[var(--text-primary)]"
                              >
                                💳 &quot;How do I pay?&quot;
