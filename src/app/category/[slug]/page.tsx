  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] pb-28 select-none">
      ...
      <header className="sticky top-0 z-40 bg-[var(--bg-primary)]/95 backdrop-blur-md border-b border-[var(--border-subtle)] px-4 py-3 flex items-center justify-between">
        ...
        {/* Right Spacer (same width as left button for mathematical centering) */}
        <div className="w-10 h-10 shrink-0 pointer-events-none" />
      </header>

      {/* ── 2. MAIN CATEGORY FEED (ASOS 2-Column Clean Grid) ───── */}
      <main className="max-w-7xl mx-auto px-2 sm:px-3 pt-2.5 pb-8">
        ...
        {categoryProducts.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-x-2 gap-y-6 sm:gap-x-3 sm:gap-y-8">
            {categoryProducts.map((product, idx) => {
              ...
              return (
                <div key={product.id} className="flex flex-col justify-between h-full group">
                  <div>
                    {/* ASOS Media Container (Vertical aspect ratio) */}
                    <div className="relative aspect-[3/4] sm:aspect-[4/5] w-full bg-[var(--bg-secondary)] overflow-hidden rounded-xl border border-[var(--border-subtle)]">
                      ...
                      {/* ASOS Top-Left Discount Micro-Badge (e.g. -75%) */}
                    ...
                    <div className="pt-2 px-0.5 space-y-0.5">
                      ...
                      <Link href={`/shop/${product.id}`} className="block">
                        <h3 className="text-xs font-normal text-[var(--text-primary)] leading-snug line-clamp-1 hover:underline">
                          {product.name}
                        </h3>
                      </Link>
                    </div>
                  ...
                  <button
                    ...
                    className="w-full mt-2.5 py-2 px-2.5 rounded-xl surface-card border border-[var(--border-subtle)] hover:border-[var(--gold-accent)] text-[var(--text-primary)] text-[10px] font-mono-luxury uppercase font-bold tracking-wider hover:bg-[var(--gold-subtle)] active:scale-95 transition-all flex items-center justify-center gap-1.5 shadow-sm cursor-pointer"
                  >
